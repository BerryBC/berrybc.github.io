...
$$
(H \otimes I)\left|{01}\right\rangle \neq (I \otimes H)\left|{01}\right\rangle
$$
...
是一个酉算符，也可以理解为量子电路中的一个“门”，作用在某个时间步。
...
表示 `a - bi`，即复数 `a + bi` 的复共轭。同理，其他元素也取其复共轭。然后，矩阵 `z*` 是通过将 `z` 的行和列互换得到的，即 `z*` 是 `z` 的转置。
